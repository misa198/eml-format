...
var emlformat = {
  ...
  getEmailAddress: function (raw) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (/^".*"$/g.test(parts[i])) {
        address.name = emlformat
          .unquoteString(parts[i])
          .replace(/"/g, "")
          .trim();
    ...
    if (!charset || charset == "utf8" || charset == "utf-8") {
      return s
        .replace(
          ...
          function (matcher, p1, p2, p3, offset, string) {
            return Buffer.from([
              parseInt(p1, 16),
              parseInt(p2, 16),
              parseInt(p3, 16),
        ...
        .replace(
          /=([\w\d]{2})=([\w\d]{2})/gi,
          function (matcher, p1, p2, offset, string) {
            return Buffer.from([parseInt(p1, 16), parseInt(p2, 16)]).toString(
              "utf8"
            );
          }
        )
    ...
    if (typeof data.to != "undefined") {
      data.headers["To"] =
        typeof data.to == "string"
          ? data.to
          : emlformat.toEmailAddress(data.to);
    }
    ...
    if (typeof data.cc != "undefined") {
      data.headers["Cc"] =
        typeof data.cc == "string"
          ? data.cc
          : emlformat.toEmailAddress(data.cc);
    }
    ...
    if (typeof data.bcc != "undefined") {
      data.headers["Bcc"] =
        typeof data.bcc == "string"
          ? data.bcc
          : emlformat.toEmailAddress(data.bcc);
    }
...
emlformat.read = function (eml, options, callback) {
  ...
  function _read(data) {
    try {
      ...
      if (data.headers["Bcc"]) {
        result.bcc = emlformat.getEmailAddress(data.headers["Bcc"]);
      }
      ...
      function _append(headers, content) {
        ...
        if (encoding == "base64") {
          if (contentType.indexOf("gbk") >= 0) {
            content = new Buffer(
              iconv.decode(new Buffer(content, "base64"), "gb2312"),
              "utf8"
            );
          } else {
...
function parseRecursive(lines, start, parent, options) {
        ...
        if (ct && /^multipart\//g.test(ct)) {
          ...
          if (b && b.length) {
            ...
            if (emlformat.verbose) {
              console.warn(
                "Multipart without boundary! " + ct.replace(/\r?\n/g, " ")
              );
            }
          }
        }
  ...
  if (
    parent.body &&
    parent.body.length &&
    parent.body[parent.body.length - 1].lines
  ) {
    complete(parent.body[parent.body.length - 1]);
  }
}

emlformat.buildAsync = async function (data) {
  return new Promise((resolve, reject) => {
    emlformat.build(data, (error, result) => {
      if (error) {
        reject(error);
      } else {
        resolve(result);
...
emlformat.readAsync = async function (eml) {
  return new Promise((resolve, reject) => {
    emlformat.read(eml, (error, result) => {
      if (error) {
        reject(error);
      } else {
        resolve(result);
